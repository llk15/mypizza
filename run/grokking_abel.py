...
DEVICE = 'cuda:7'
C = cfg.n_vocab
cfg.project_base = 'grokking_abel'
...
for dataset_try in range(1):
    ...
    for cfg.model_type in ['A', 'B', 'C', 'T']:
        for cfg.tied_embeddings in [True,False]:
            for model_try in range(5):
                cfg.model_seed = random.randint(1000,9999)
                cfg.group = '_'.join([cfg.model_type, str(int(cfg.tied_embeddings))])
